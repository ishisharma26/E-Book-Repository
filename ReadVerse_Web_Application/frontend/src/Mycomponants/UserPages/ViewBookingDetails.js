...
const ViewBookingDetails = () => {
    ...
    const DownloadFunction = (productId) => {
        if (!productId) {
            return alert('Product ID does not exist!');
        }
    
        // Make a GET request to download the PDF
        axios.get(`http://localhost:5000/user/download-pdf/${productId}`, {
      ...
      <div className="content">
        <form action="#">
          <UserDetails>
            ...
            {/* <InputBox>
              <span className="details"> <h3>Delevery Address<span className="details">  : </span> </h3></span>
            </InputBox>
            <InputBox>
              <span className="details"> <h5 >{booking?.add}  </h5>
                    </span>
            </InputBox> */}
        ...
        <tbody>
          {booking?.products?.map((p, index) => (
            <tr key={index}>
              ...
              <td ><FontAwesomeIcon onClick={()=>DownloadFunction(p?.productId?._id)} style={{margin:'20px'}} icon={faDownload} /></td>
            </tr>
          ))}
        </tbody>
      </table>
