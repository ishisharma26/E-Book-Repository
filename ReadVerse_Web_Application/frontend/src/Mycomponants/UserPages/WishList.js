...
function WishList() {
    ...
    useEffect (()=>{
        ...
        .then((result)=>{
            console.log(result.data.result.products)
            setRecord(result.data.result.products);
    ...
    const handleFilter = (e) => {
        ...
        const newdata = data.filter(row => {
            const firstNameMatch = row?.productId?.title?.toLowerCase().includes(searchTerm);
            const firstNameMatch2 = row?.productId?.genreId?.name?.toLowerCase().includes(searchTerm);
            const firstNameMatch3 = row?.productId?.price >= 0 && row?.productId?.price <= newPriceRange; // Filter by updated price range
            return firstNameMatch || firstNameMatch2 || firstNameMatch3;
        });
        setRecord(newdata);
    };
    ...
    const HendleClick=(row)=>{
        navigate("/userItemDetails",{state:{book:row}});
    }
    ...
    const AddToCart=(productId)=>{
     ...
     const RemoveToWishList=(productId)=>{
       ...
       if(confirmed){
        ...
        axios.post(`http://localhost:5000/user/removeWishList/${productId}`)
        .then((result)=>{
          console.log(result.data)
           if(result.data.message==='Product removed successfully.'){
            setData(data.filter((p)=> p?.productId?._id!==productId )); 
            setRecord(data.filter((p)=> p?.productId?._id!==productId ));
           }
           else{
     ...
     const RemoveToWishList2=(productId)=>{
        if(!productId){
            alert("Please Fill All filds")
            return
        }
                                ...
                                <div class="row g-4 justify-content-center">
                                    {
                                       currentItems.map((r)=>(
                                        ...
                                        <div   class="rounded position-relative fruite-item">
                                            <div onClick={() => HendleClick(r?.productId)} class="fruite-img">
                                                <img style={{height:"200px"}}  src={`http://localhost:5000/images/${r?.productId?.pic}`} class="img-fluid w-100 rounded-top" alt=""/>
                                            </div>
                                            <div onClick={() => HendleClick(r?.productId)} class="text-white bg-secondary px-3 py-1 rounded position-absolute" style={{top: "10px", left: "10px"}}>
                                          {r?.productId?.genreId?.name}
                                          </div>

                             <div class="p-4 border border-secondary border-top-0 rounded-bottom">
                                     <h4>{r?.productId?.title?.toUpperCase()}</h4>
                                      <div style={{height:"50px",width:"100%", overflow: "hidden"}}>
                                         <p style={{height: "100%", overflow: "hidden"}}>{r?.productId?.dis}  </p>
                                        </div>
                                    <div class="d-flex justify-content-between flex-lg-wrap">
                                        <p class="text-dark fs-5 fw-bold mb-0"> ₹ {r?.productId?.price}</p>
                                       <button  onClick={() => {AddToCart(r?.productId?._id);RemoveToWishList2(r?.productId?._id);
}} class="btn border border-secondary rounded-pill px-3 text-primary"><i class="fa fa-shopping-bag me-2 text-primary"></i> Add to cart</button>
                                       <button style={{margin:"10px"}}  onClick={()=>RemoveToWishList(r?.productId?._id)}  class="btn border border-secondary rounded-pill px-4 py-2 mb-4 text-primary"><i class="fas fa-heart fa-fw me-2 text-primary"></i> Remove From Wishlist</button>
