...
const UpdateAuthorProfile = () => {
    ...
    // Function to handle form submission
      const handleSubmit = async (event) => {
          ...
          axios.put(`http://localhost:5000/author/updateProfile`, formData, {
              headers: {
          ...
          .then((result) => {
              ...
              if (result.data.message=== "Author successfully updated") {
                  navigate("/authorMyProfile");
    ...
    useEffect(()=>{
        axios.get(`http://localhost:5000/author/getAuthor`)
        .then((result)=>{
            console.log(result)
          if(result.data.message==="We get all Author"){
...
export default UpdateAuthorProfile;
